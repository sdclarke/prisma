...
public class RubiksCubeOptimalXCross implements Tip {
  ...
  public String getTipDescription() {
    return i18n("tip.RUBIKS-CUBE-OPTIMAL-X-CROSS");
  }
  ...
  public String getTip(Scramble scramble) {
    ...
    tip.append(i18n("tip.RUBIKS-CUBE-OPTIMAL-X-CROSS.optimal_x_cross_on_u")).append(":\n");
    tip.append(getOptimalSolutions(stateU, "x2 "));
    tip.append("\n");

    // x-cross on D
    State stateD = state;
    tip.append(i18n("tip.RUBIKS-CUBE-OPTIMAL-X-CROSS.optimal_x_cross_on_d")).append(":\n");
    tip.append(getOptimalSolutions(stateD, ""));
    tip.append("\n");

    // x-cross on L
    State stateL = z.multiply(state).multiply(z).multiply(z).multiply(z);
    tip.append(i18n("tip.RUBIKS-CUBE-OPTIMAL-X-CROSS.optimal_x_cross_on_l")).append(":\n");
    tip.append(getOptimalSolutions(stateL, "z' "));
    tip.append("\n");

    // x-cross on R
    State stateR = z.multiply(z).multiply(z).multiply(state).multiply(z);
    tip.append(i18n("tip.RUBIKS-CUBE-OPTIMAL-X-CROSS.optimal_x_cross_on_r")).append(":\n");
    tip.append(getOptimalSolutions(stateR, "z "));
    tip.append("\n");

    // x-cross on F
    State stateF = x.multiply(state).multiply(x).multiply(x).multiply(x);
    tip.append(i18n("tip.RUBIKS-CUBE-OPTIMAL-X-CROSS.optimal_x_cross_on_f")).append(":\n");
    tip.append(getOptimalSolutions(stateF, "x' "));
    tip.append("\n");

    // x-cross on B
    State stateB = x.multiply(x).multiply(x).multiply(state).multiply(x);
    tip.append(i18n("tip.RUBIKS-CUBE-OPTIMAL-X-CROSS.optimal_x_cross_on_b")).append(":\n");
  ...
  private String getOptimalSolutions(State state, String prefix) {
    ...
    for (int i = 0; i < solutions.size(); i++) {
      if (solutions.get(i).length == minLength) {
        output.append(
            String.format("  %s%s\n", prefixes.get(i), StringUtils.join(" ", solutions.get(i))));
      }
    }
