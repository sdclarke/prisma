...
public class SkewbSolver {
  public static class State {
    ...
    public State(
        byte[] facesPermutation,
        byte[] freeCornersPermutation,
        byte[] freeCornersOrientation,
    ...
    public State multiply(State move) {
      ...
      for (int i = 0; i < freeCornersPermutation.length; i++) {
        freeCornersPermutation[i] = this.freeCornersPermutation[move.freeCornersPermutation[i]];
      }
      ...
      for (int i = 0; i < freeCornersOrientation.length; i++) {
        freeCornersOrientation[i] =
            (byte)
                ((this.freeCornersOrientation[move.freeCornersPermutation[i]]
                        + move.freeCornersOrientation[i])
                    % 3);
      }
      ...
      for (int i = 0; i < freeCornersOrientation.length; i++) {
        fixedCornersOrientation[i] =
            (byte) ((this.fixedCornersOrientation[i] + move.fixedCornersOrientation[i]) % 3);
      }
  ...
  private void initialize() {
    ...
    State moveL =
        new State(
            new byte[] {1, 4, 2, 3, 0, 5},
            new byte[] {2, 0, 1, 3},
            new byte[] {2, 2, 2, 0},
            new byte[] {1, 0, 0, 0});
    State moveR =
        new State(
            new byte[] {3, 1, 0, 2, 4, 5},
            new byte[] {1, 3, 2, 0},
            new byte[] {2, 2, 0, 2},
            new byte[] {0, 1, 0, 0});
    State moveD =
        new State(
            new byte[] {0, 1, 2, 4, 5, 3},
            new byte[] {0, 2, 3, 1},
            new byte[] {0, 2, 2, 2},
            new byte[] {0, 0, 0, 1});
    State moveB =
        new State(
            new byte[] {0, 2, 5, 3, 4, 1},
            new byte[] {3, 1, 0, 2},
            new byte[] {2, 0, 2, 2},
            new byte[] {0, 0, 1, 0});
    ...
    for (int i = 0; i < this.facesPermutationMove.length; i++) {
      State state =
          new State(
              IndexMapping.indexToEvenPermutation(i, 6), new byte[4], new byte[4], new byte[4]);
      for (int j = 0; j < this.moves.length; j++) {
        this.facesPermutationMove[i][j] =
            IndexMapping.evenPermutationToIndex(state.multiply(this.moves[j]).facesPermutation);
      }
    }
    ...
    for (int i = 0; i < this.freeCornersPermutationMove.length; i++) {
      State state =
          new State(
              new byte[6], IndexMapping.indexToEvenPermutation(i, 4), new byte[4], new byte[4]);
      for (int j = 0; j < this.moves.length; j++) {
    ...
    for (int i = 0; i < this.fixedCornersOrientationMove.length; i++) {
      State state =
          new State(
              new byte[6], new byte[4], new byte[4], IndexMapping.indexToOrientation(i, 3, 4));
      for (int j = 0; j < this.moves.length; j++) {
    ...
    // distance table
    this.distance =
        new int[this.N_FACES_PERMUTATIONS][this.N_FREE_CORNERS_PERMUTATION]
            [this.N_FREE_CORNERS_ORIENTATION][this.N_FIXED_CORNERS_ORIENTATION];
            ...
            for (int m = 0; m < this.distance[i][j][k].length; m++) {
              if (this.distance[i][j][k][m] == depth) {
                for (int moveIndex = 0; moveIndex < this.moves.length; moveIndex++) {
                  ...
                  if (this.distance[nextFacesPermutation][nextFreeCornersPemutation][
                          nextFreeCornersOrientation][nextFixedCornersOrientation]
                      == -1) {
                    this.distance[nextFacesPermutation][nextFreeCornersPemutation][
                            nextFreeCornersOrientation][nextFixedCornersOrientation] =
                        depth + 1;
                    nVisited++;
                  }
  ...
  public String[] solve(State state) {
    ...
    int freeCornersOrientation =
        IndexMapping.zeroSumOrientationToIndex(state.freeCornersOrientation, 3);
    int fixedCornersOrientation = IndexMapping.orientationToIndex(state.fixedCornersOrientation, 3);

    for (; ; ) {
      if (this.distance[facesPermutation][freeCornersPermutation][freeCornersOrientation][
              fixedCornersOrientation]
          == 0) {
        break;
      }
      ...
      for (int k = 0; k < this.moves.length; k++) {
        ...
        int nextFixedCornersOrientation =
            this.fixedCornersOrientationMove[fixedCornersOrientation][k];

        if (this.distance[nextFacesPermutation][nextFreeCornersPemutation][
                nextFreeCornersOrientation][nextFixedCornersOrientation]
            == this.distance[facesPermutation][freeCornersPermutation][freeCornersOrientation][
                    fixedCornersOrientation]
                - 1) {
  ...
  public State getRandomState(Random random) {
    ...
    for (; ; ) {
      int indexFacesPermutation = random.nextInt(this.N_FACES_PERMUTATIONS);
      int indexFreeCornersPermutation = random.nextInt(this.N_FREE_CORNERS_PERMUTATION);
      int indexFreeCornersOrientation = random.nextInt(this.N_FREE_CORNERS_ORIENTATION);
      int indexFixedCornersOrientation = random.nextInt(this.N_FIXED_CORNERS_ORIENTATION);

      if (this.distance[indexFacesPermutation][indexFreeCornersPermutation][
              indexFreeCornersOrientation][indexFixedCornersOrientation]
          == -1) {
        continue;
      }
