...
public class RubiksPocketCubeSolver {
  public static class State {
    ...
    public State multiply(State move) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        resultOrientation[i] =
            (byte) ((this.orientation[move.permutation[i]] + move.orientation[i]) % 3);
      }
    ...
    static {
      id = new State(new byte[] {0, 1, 2, 3, 4, 5, 6, 7}, new byte[] {0, 0, 0, 0, 0, 0, 0, 0});
    }
  }
  ...
  public RubiksPocketCubeSolver(int minScrambleLenght, String[] generatingSet) {
    ...
    table.put(
        "U", new State(new byte[] {3, 0, 1, 2, 4, 5, 6, 7}, new byte[] {0, 0, 0, 0, 0, 0, 0, 0}));
    table.put(
        "D", new State(new byte[] {0, 1, 2, 3, 5, 6, 7, 4}, new byte[] {0, 0, 0, 0, 0, 0, 0, 0}));
    table.put(
        "L", new State(new byte[] {4, 1, 2, 0, 7, 5, 6, 3}, new byte[] {2, 0, 0, 1, 1, 0, 0, 2}));
    table.put(
        "R", new State(new byte[] {0, 2, 6, 3, 4, 1, 5, 7}, new byte[] {0, 1, 2, 0, 0, 2, 1, 0}));
    table.put(
        "F", new State(new byte[] {0, 1, 3, 7, 4, 5, 2, 6}, new byte[] {0, 0, 1, 2, 0, 0, 2, 1}));
    table.put(
        "B", new State(new byte[] {1, 5, 2, 3, 0, 4, 6, 7}, new byte[] {1, 2, 0, 0, 2, 1, 0, 0}));
    ...
    for (int i = 0; i < this.permutationMove.length; i++) {
      ...
      for (int j = 0; j < this.moves.size(); j++) {
        this.permutationMove[i][j] =
            IndexMapping.permutationToIndex(state.multiply(this.moves.get(j)).permutation);
      }
    }
  ...
  private boolean search(
      int permutation, int orientation, int depth, ArrayList<String> solution, int lastFace) {
    if (depth == 0) {
