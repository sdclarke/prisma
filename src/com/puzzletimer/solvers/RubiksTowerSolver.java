...
public class RubiksTowerSolver {
  public static class State {
    ...
    public State multiply(State move) {
      ...
      for (int i = 0; i < 8; i++) {
        orientation[i] =
            (byte) ((this.orientation[move.edgesPermutation[i]] + move.orientation[i]) % 3);
  ...
  private void initialize() {
    ...
    this.moveNames2 =
        new String[] {
          "Uw", "Uw2", "Uw'", "Dw", "Dw2", "Dw'", "Lw2", "Rw2", "Fw2", "Bw2", "U", "U2", "U'", "D",
          "D2", "D'",
        };

    this.faces2 =
        new int[] {
          0, 0, 0, 1, 1, 1, 2, 3, 4, 5, 6, 6, 6, 7, 7, 7,
        };
    ...
    for (int i = 0; i < this.orientationMove.length; i++) {
      State state =
          new State(IndexMapping.indexToZeroSumOrientation(i, 3, 8), new byte[8], new byte[8]);
      for (int j = 0; j < this.moves1.length; j++) {
        this.orientationMove[i][j] =
            IndexMapping.zeroSumOrientationToIndex(state.multiply(this.moves1[j]).orientation, 3);
      }
    }
    ...
    for (int i = 0; i < this.edgesPermutationMove.length; i++) {
      ...
      for (int j = 0; j < this.moves2.length; j++) {
        this.edgesPermutationMove[i][j] =
            IndexMapping.permutationToIndex(state.multiply(this.moves2[j]).edgesPermutation);
      }
    }
    ...
    for (int i = 0; i < this.cornersPermutationMove.length; i++) {
      ...
      for (int j = 0; j < this.moves2.length; j++) {
        this.cornersPermutationMove[i][j] =
            IndexMapping.permutationToIndex(state.multiply(this.moves2[j]).cornersPermutation);
      }
    }
    ...
    this.edgesPermutationDistance =
        new byte[this.N_EDGES_PERMUTATIONS][this.N_CORNERS_COMBINATIONS];
          ...
          if (this.edgesPermutationDistance[i][j] == depth) {
            for (int k = 0; k < this.moves2.length; k++) {
              ...
              if (this.edgesPermutationDistance[nextPermutation][nextCombination] < 0) {
                this.edgesPermutationDistance[nextPermutation][nextCombination] =
                    (byte) (depth + 1);
                nVisited++;
              }
            }
    ...
    this.cornersPermutationDistance =
        new byte[this.N_CORNERS_PERMUTATIONS][this.N_EDGES_COMBINATIONS];
          ...
          if (this.cornersPermutationDistance[i][j] == depth) {
            for (int k = 0; k < this.moves2.length; k++) {
              ...
              if (this.cornersPermutationDistance[nextPermutation][nextCombination] < 0) {
                this.cornersPermutationDistance[nextPermutation][nextCombination] =
                    (byte) (depth + 1);
                nVisited++;
              }
            }
  ...
  private boolean search(int orientation, int depth, ArrayList<Integer> solution, int lastFace) {
    ...
    if (this.orientationDistance[orientation] <= depth) {
      for (int i = 0; i < this.moves1.length; i++) {
        ...
        solution.add(i);
        if (search(this.orientationMove[orientation][i], depth - 1, solution, this.faces1[i])) {
          return true;
        }
  ...
  private boolean search2(
      ...
      int edgesCombination,
      int cornersPermutation,
      int cornersCombination,
      int depth,
  ...
  public State getRandomState(Random random) {
    byte[] orientation =
        IndexMapping.indexToZeroSumOrientation(random.nextInt(this.N_ORIENTATIONS), 3, 8);
    byte[] edgesPermutation =
        IndexMapping.indexToPermutation(random.nextInt(this.N_EDGES_PERMUTATIONS), 8);
    byte[] cornersPermutation =
        IndexMapping.indexToPermutation(random.nextInt(this.N_CORNERS_PERMUTATIONS), 8);
