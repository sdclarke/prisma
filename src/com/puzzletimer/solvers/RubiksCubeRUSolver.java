...
public class RubiksCubeRUSolver {
  ...
  static {
    int[] goalIndices = stateToIndices(RubiksCubeSolver.State.id);
        ...
        for (int j = 0; j < cornersDistance[i].length; j++) {
          ...
          for (int k = 0; k < cornersPermutationMove[i].length; k++) {
            ...
            if (cornersDistance[nextPermutation][nextOrientation] < 0) {
              cornersDistance[nextPermutation][nextOrientation] = (byte) (distance + 1);
              nVisited++;
            }
          }
        }
        ...
        for (int j = 0; j < edgesDistance[i].length; j++) {
          ...
          for (int k = 0; k < edgesPermutationMove[i].length; k++) {
            ...
            if (edgesDistance[nextPermutation][nextOrientation] < 0) {
              edgesDistance[nextPermutation][nextOrientation] = (byte) (distance + 1);
              nVisited++;
            }
          }
        }
  ...
  public static String[] solve(State state) {
    ...
    for (int depth = 0; ; depth++) {
      ...
      if (search(indices[0], indices[1], indices[2], indices[3], depth, solution)) {
  ...
  public static State getRandomState(Random random) {
    for (; ; ) {
      ...
      State state =
          indicesToState(
              new int[] {
                cornersPermutation, cornersOrientation, edgesPermutation, edgesOrientation,
              });

      if (permutationSign(state.cornersPermutation) == permutationSign(state.edgesPermutation)) {
        return state;
      }
    }
  }
