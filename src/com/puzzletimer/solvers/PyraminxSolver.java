...
public class PyraminxSolver {
  public static class State {
    ...
    public State multiply(State move) {
      ...
      for (int i = 0; i < 4; i++) {
        verticesOrientation[i] =
            (byte) ((this.verticesOrientation[i] + move.verticesOrientation[i]) % 3);
      }
      ...
      for (int i = 0; i < 6; i++) {
        ...
        edgesOrientation[i] =
            (byte)
                ((this.edgesOrientation[move.edgesPermutation[i]] + move.edgesOrientation[i]) % 2);
      }
  ...
  private void initialize() {
    ...
    State moveU =
        new State(
            new byte[] {1, 0, 0, 0},
            new byte[] {1, 0, 0, 0},
            new byte[] {2, 0, 1, 3, 4, 5},
            new byte[] {0, 0, 0, 0, 0, 0});
    State moveL =
        new State(
            new byte[] {0, 1, 0, 0},
            new byte[] {0, 1, 0, 0},
            new byte[] {0, 1, 5, 3, 2, 4},
            new byte[] {0, 0, 1, 0, 0, 1});
    State moveR =
        new State(
            new byte[] {0, 0, 1, 0},
            new byte[] {0, 0, 1, 0},
            new byte[] {0, 4, 2, 1, 3, 5},
            new byte[] {0, 1, 0, 0, 1, 0});
    State moveB =
        new State(
            new byte[] {0, 0, 0, 1},
            new byte[] {0, 0, 0, 1},
            new byte[] {3, 1, 2, 5, 4, 0},
    ...
    for (int i = 0; i < this.tipsOrientationMove.length; i++) {
      State state =
          new State(
              IndexMapping.indexToOrientation(i, 3, 4), new byte[4], new byte[6], new byte[6]);
      for (int j = 0; j < this.moves.length; j++) {
        this.tipsOrientationMove[i][j] =
            IndexMapping.orientationToIndex(state.multiply(this.tipMoves[j]).tipsOrientation, 3);
      }
    }
    ...
    for (int i = 0; i < this.verticesOrientationMove.length; i++) {
      State state =
          new State(
              new byte[4], IndexMapping.indexToOrientation(i, 3, 4), new byte[6], new byte[6]);
      for (int j = 0; j < this.moves.length; j++) {
        this.verticesOrientationMove[i][j] =
            IndexMapping.orientationToIndex(state.multiply(this.moves[j]).verticesOrientation, 3);
      }
    }
    ...
    for (int i = 0; i < this.edgesPermutationMove.length; i++) {
      State state =
          new State(
              new byte[4], new byte[4], IndexMapping.indexToEvenPermutation(i, 6), new byte[6]);
      for (int j = 0; j < this.moves.length; j++) {
        this.edgesPermutationMove[i][j] =
            IndexMapping.evenPermutationToIndex(state.multiply(this.moves[j]).edgesPermutation);
      }
    }
    ...
    for (int i = 0; i < this.edgesOrientationMove.length; i++) {
      State state =
          new State(
              ...
              new byte[4],
              new byte[6],
              IndexMapping.indexToZeroSumOrientation(i, 2, 6));
      for (int j = 0; j < this.moves.length; j++) {
        this.edgesOrientationMove[i][j] =
            IndexMapping.zeroSumOrientationToIndex(
                state.multiply(this.moves[j]).edgesOrientation, 2);
      }
    }
  ...
  private boolean searchTips(
      int tipsOrientation, int depth, ArrayList<String> solution, int lastVertex) {
    if (depth == 0) {
    ...
    if (this.tipsOrientationDistance[tipsOrientation] <= depth) {
      for (int i = 0; i < this.tipMoves.length; i++) {
        ...
        if (searchTips(this.tipsOrientationMove[tipsOrientation][i], depth - 1, solution, i / 2)) {
          return true;
        }
  ...
  public String[] generate(State state) {
    ...
    for (int i = 0; i < tipsSolution.length; i++) {
      sequence[solution.length + i] =
          inverseMoveNames.get(tipsSolution[tipsSolution.length - 1 - i]);
    }
  ...
  public State getRandomState(Random random) {
    int tipsOrientation = random.nextInt(this.N_TIPS_ORIENTATIONS);
    int verticesOrientation = random.nextInt(this.N_VERTICES_ORIENTATIONS);
    int edgesPermutation = random.nextInt(this.N_EDGES_PERMUTATIONS);
    int edgesOrientation = random.nextInt(this.N_EDGES_ORIENTATIONS);
