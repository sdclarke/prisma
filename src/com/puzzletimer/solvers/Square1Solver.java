...
public class Square1Solver {
  ...
  private void initialize() {
    ...
    State moveTwist =
        new State(
            new byte[] {
              0, 1, 19, 18, 17, 16, 15, 14, 8, 9, 10, 11, 12, 13, 7, 6, 5, 4, 3, 2, 20, 21, 22, 23,
            });
        ...
        for (int i = 0; i < this.moves1.length; i++) {
          ...
          HashMap<Integer, Integer> distanceTable =
              isEvenPermutation(next.getPiecesPermutation())
                  ? this.evenShapeDistance
                  : this.oddShapeDistance;
    ...
    // moves
    CubeState move30 =
        new CubeState(new byte[] {3, 0, 1, 2, 4, 5, 6, 7}, new byte[] {3, 0, 1, 2, 4, 5, 6, 7});
    CubeState move03 =
        new CubeState(new byte[] {0, 1, 2, 3, 5, 6, 7, 4}, new byte[] {0, 1, 2, 3, 5, 6, 7, 4});
    CubeState moveTwistTop =
        new CubeState(new byte[] {0, 6, 5, 3, 4, 2, 1, 7}, new byte[] {6, 5, 2, 3, 4, 1, 0, 7});
    CubeState moveTwistBottom =
          ...
          if (this.cornersDistance[i][j] == depth) {
            for (int k = 0; k < this.moves2.length; k++) {
              ...
              if (this.cornersDistance[nextCornerPermutation][nextEdgeCombination] < 0) {
                this.cornersDistance[nextCornerPermutation][nextEdgeCombination] =
                    (byte) (depth + 1);
                nVisited++;
              }
            }
          ...
          if (this.edgesDistance[i][j] == depth) {
            for (int k = 0; k < this.moves2.length; k++) {
              ...
              if (this.edgesDistance[nextEdgesPermutation][nextCornersCombination] < 0) {
                this.edgesDistance[nextEdgesPermutation][nextCornersCombination] =
                    (byte) (depth + 1);
                nVisited++;
              }
            }
  ...
  private int[] solution(State state) {
    ...
    for (int depth = 0; ; depth++) {
      ...
      if (search(
          state, isEvenPermutation(state.getPiecesPermutation()), depth, solution1, solution2)) {
        ...
        int[][] phase2MoveMapping = {
          {2}, {5}, {8}, {13}, {16}, {19}, {0, 22, 10}, {21, 22, 11},
        };
  ...
  private boolean search2(
    ...
    if (this.cornersDistance[cornersPermutation][edgesCombination] <= depth
      ...
      for (int i = 0; i < this.moves2.length; i++) {
        if (solution.length - depth - 1 >= 0
            && solution[solution.length - depth - 1] / 3 == i / 3) {
          continue;
        }
