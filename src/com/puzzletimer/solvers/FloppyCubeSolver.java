...
public class FloppyCubeSolver {
  ...
  public static String[] solve(State state) {
    ...
    for (; ; ) {
      ...
      int edgesOrientationIndex = IndexMapping.orientationToIndex(state.edgesOrientation, 2);
  ...
  public static State getRandomState(Random random) {
    for (; ; ) {
      int cornersPermutationIndex = random.nextInt(N_CORNERS_PERMUTATION);
      int edgesOrientationIndex = random.nextInt(N_EDGES_ORIENTATION);
