...
public class Main {
  ...
  private static final String DATABASE_RESOURCE_LOCATION = "/com/puzzletimer/resources/database/";
  ...
  public Main() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          frame,
          i18n("main.database_driver_load_error"),
      ...
      try {
        ...
        Reader script =
            new InputStreamReader(
                getClass().getResourceAsStream(DATABASE_RESOURCE_LOCATION + "puzzletimer0.3.sql"));
    ...
    try {
      ...
      ResultSet resultSet =
          statement.executeQuery("SELECT VALUE FROM CONFIGURATION WHERE KEY = 'VERSION'");
    ...
    for (versionIndex++; versionIndex <= maxVersionIndex; versionIndex++) {
      try {
        ...
        Reader script =
            new InputStreamReader(
                getClass().getResourceAsStream(DATABASE_RESOURCE_LOCATION + scriptName));
        RunScript.execute(connection, script);
      } catch (SQLException e) {
        JFrame frame = new JFrame();
        JOptionPane.showMessageDialog(
    ...
    this.configurationManager.addListener(
        new ConfigurationManager.Listener() {
          @Override
                ...
                Main.this.messageManager.enqueueMessage(
                    ...
                    String.format(
                        ...
                        SolutionUtils.formatMinutes(
                            measures[i].getValue(),
                            Main.this.configurationManager.getConfiguration("TIMER-PRECISION"),
                            measures[i].getRound()),
                        descriptions[i]));
              }
            }
    ...
    UUID currentCategoryId =
        UUID.fromString(this.configurationManager.getConfiguration("CURRENT-CATEGORY"));
    ...
    this.categoryManager.addListener(
        new CategoryManager.Listener() {
          ...
          public void currentCategoryChanged(Category category) {
            Main.this.configurationManager.setConfiguration(
                "CURRENT-CATEGORY", category.getCategoryId().toString());
    ...
    this.scrambleManager =
        new ScrambleManager(
            this.scramblerProvider, this.scramblerProvider.get(currentCategory.getScramblerId()));
    this.categoryManager.addListener(
        new CategoryManager.Listener() {
          @Override
          public void currentCategoryChanged(Category category) {
    ...
    // solution DAO
    this.solutionDAO =
        new SolutionDAO(connection, this.scramblerProvider, this.scrambleParserProvider);

    // solution manager
    this.solutionManager = new SolutionManager();
    this.solutionManager.addListener(
        new SolutionManager.Listener() {
          @Override
    ...
    this.sessionManager.setDailySessionEnabled(
        Main.this.configurationManager.getConfiguration("DAILYSESSION-ENABLED").equals("TRUE"));
        ...
        new Runnable() {
          public void run() {
            ...
            if (main.configurationManager.getConfiguration("DAILYSESSION-ENABLED").equals("TRUE")) {
              Solution[] sols =
                  main.solutionDAO.getCurrentSession(main.categoryManager.getCurrentCategory());
              for (Solution solution : sols) {
                main.sessionManager.addSolution(solution);
              }
            }

            if (!mainFrame.hasUpdate()) mainFrame.setVisible(true);
          }
        });
  }
}
