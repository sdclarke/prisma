...
public class Square1 implements Puzzle {
  ...
  public Mesh getScrambledPuzzleMesh(ColorScheme colorScheme, String[] sequence) {
    ...
    Plane planeR =
        new Plane(
            new Vector3(0, 0, 0), Matrix44.rotationY(-Math.PI / 12).mul(new Vector3(1, 0, 0)));

    Plane p1 =
        new Plane(new Vector3(0, 0, 0), Matrix44.rotationY(Math.PI / 12).mul(new Vector3(1, 0, 0)));

    Plane p2 =
        new Plane(
            new Vector3(0, 0, 0), Matrix44.rotationY(-Math.PI / 12).mul(new Vector3(0, 0, 1)));

    Plane p3 =
        new Plane(new Vector3(0, 0, 0), Matrix44.rotationY(Math.PI / 12).mul(new Vector3(0, 0, 1)));

    Mesh mesh =
        cube.cut(planeD, 0.01)
            .cut(planeU, 0.01)
            .cut(planeR, 0.01)
            .cut(p1, 0.01)
            .cut(p2, 0.01)
    ...
    return cube.transform(Matrix44.rotationY(-Math.PI / 6))
