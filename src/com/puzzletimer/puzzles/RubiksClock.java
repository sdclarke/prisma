...
public class RubiksClock implements Puzzle {
  ...
  private Mesh hand(
      double radius1, int nVertices1, double radius2, int nVertices2, double height, Color color) {
  ...
  public Mesh getScrambledPuzzleMesh(ColorScheme colorScheme, String[] sequence) {
    ...
    Mesh handBackground =
        hand(0.04, 8, 0.001, 3, 0.16, colorScheme.getFaceColor("HAND-BACKGROUND").getColor())
            .transform(Matrix44.translation(new Vector3(0, 0, -0.025)));

    Mesh handForeground =
        hand(0.025, 8, 0.001, 3, 0.11, colorScheme.getFaceColor("HAND-FOREGROUND").getColor())
            .transform(Matrix44.translation(new Vector3(0, 0, -0.05)));
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        Matrix44 transformation =
            Matrix44.translation(new Vector3(0.5 * (j - 1), 0.5 * (1 - i), 0))
                .mul(Matrix44.rotationZ(Math.PI / 6 * state.clocks[3 * i + j]));

        front =
            front.union(
                circle(0.225, 32, colorScheme.getFaceColor("FRONT").getColor())
                    .union(hands)
      ...
      for (int j = 0; j < 2; j++) {
        ...
        Matrix44 transformation =
            Matrix44.translation(
                new Vector3(0.5 * (j - 0.5), 0.5 * (0.5 - i), pinDown ? 0.0 : -0.1));

        Color pinColor = colorScheme.getFaceColor(pinDown ? "PIN-DOWN" : "PIN-UP").getColor();
        front = front.union(circle(0.05, 16, pinColor).transform(transformation));
      }
    }
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        Matrix44 transformation =
            Matrix44.translation(new Vector3(0.5 * (j - 1), 0.5 * (1 - i), 0))
                .mul(Matrix44.rotationZ(Math.PI / 6 * state.clocks[9 + 3 * i + j]));

        back =
            back.union(
                circle(0.225, 32, colorScheme.getFaceColor("BACK").getColor())
                    .union(hands)
                    .transform(transformation));
      }
    }
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        ...
        Color pinColor = colorScheme.getFaceColor(pinDown ? "PIN-DOWN" : "PIN-UP").getColor();
        back = back.union(circle(0.05, 16, pinColor).transform(transformation));
      }
    }
    ...
    return front
        .transform(Matrix44.translation(new Vector3(0, 0, -0.1)))
        .union(back.transform(Matrix44.translation(new Vector3(0, 0, 0.1))));
  }
}
