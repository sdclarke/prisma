...
public class SolutionDAO {
  ...
  public Solution[] getAll(Category category) {
    ...
    ScrambleParser scramblerParser =
        this.scrambleParserProvider.get(scrambler.getScramblerInfo().getPuzzleId());
    ...
    try {
      PreparedStatement statement =
          this.connection.prepareStatement(
              "SELECT SOLUTION_ID, CATEGORY_ID, SCRAMBLER_ID, SEQUENCE, START, END, PENALTY,"
                  + " COMMENT FROM SOLUTION WHERE CATEGORY_ID = ? ORDER BY START DESC");
      ...
      while (resultSet.next()) {
        ...
        Solution solution =
            new Solution(
                solutionId, categoryId, scramble, new Timing(start, end), penalty, comment);
  ...
  public void insert(Solution[] solutions) {
    try {
      ...
      PreparedStatement statement =
          this.connection.prepareStatement("INSERT INTO SOLUTION VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
  ...
  public Solution[] getCurrentSession(Category category) {
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        Scramble scramble = new Scramble(scramblerId, scramblerParser.parse(sequence));
        Solution solution =
            new Solution(solutionId, categoryId, scramble, new Timing(start, end), penalty, "");
  ...
  public void delete(Solution solution) {
    try {
      PreparedStatement statement =
          this.connection.prepareStatement("DELETE FROM SOLUTION WHERE SOLUTION_ID = ?");
