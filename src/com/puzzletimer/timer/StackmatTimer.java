  ...
  public void run() {
    ...
    while (this.running) {
      ...
      for (this.baudRateOffset = 0; this.baudRateOffset < 25; this.baudRateOffset++) {
        ...
        this.period = this.sampleRate / (double) this.baudRate;
        for (offset = 0;
            offset + (this.hasSixDigits ? 0.132015 : 0.119181) * this.sampleRate < buffer.length;
            offset++) {
        ...
        for (int j = 0; j < this.period; j++) {
          ...
          if (isValidPacket(data)) {
            if (baudRateHistogram.containsKey(this.previousBaudRate + this.baudRateOffset + i)) {
              baudRateHistogram.put(
                  this.previousBaudRate + this.baudRateOffset + i,
                  baudRateHistogram.get(this.previousBaudRate + this.baudRateOffset + i) + 1);
            } else {
...
public class StackmatTimer implements StackmatTimerReaderListener, Timer {
  ...
  public StackmatTimer(
      ...
      StackmatDeveloperFrame stackmatDeveloperFrame) {
  ...
  public void start() {
    ...
    this.repeater.schedule(
        new TimerTask() {
          @Override
          public void run() {
            switch (StackmatTimer.this.state) {
              case RUNNING:
                if (smoothTimingEnabled)
