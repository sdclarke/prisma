...
public class GraphPanel extends JPanel {
  ...
  public void paintComponent(Graphics g) {
    ...
    for (int i = 0; i < nVerticalTicks; i++) {
      ...
      int y =
          (int)
              (getHeight()
                  - hBase
                  - (i + 0.5) * (getHeight() - hBase) / nVerticalTicks
                  + height / 2
                  - 1);
    ...
    double hTickInterval =
        (double) (this.startIntervalEnd - this.startIntervalStart) / nHorizontalTicks;
    for (int i = 0; i < nHorizontalTicks; i++) {
    ...
    for (int i = 0; i < this.solutionTimes.size(); i++) {
      int bin =
          (int)
              ((nBins - 1)
                  * (this.startTimes.get(i) - this.startIntervalStart)
                  / (this.startIntervalEnd - this.startIntervalStart));
          ...
          int y =
              (int)
                  (getHeight()
                      - hBase
                      - (getHeight() - hBase)
                          * (mean - this.solutionIntervalStart)
                          / (this.solutionIntervalEnd - this.solutionIntervalStart));
