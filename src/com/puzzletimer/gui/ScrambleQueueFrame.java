...
public class ScrambleQueueFrame extends JFrame {
  ...
  public ScrambleQueueFrame(
    ...
    categoryManager.addListener(
        new CategoryManager.Listener() {
          ...
          public void categoriesUpdated(Category[] categories, Category currentCategory) {
            ...
            Scrambler currentScrambler =
                scramblerProvider.get(categoryManager.getCurrentCategory().getScramblerId());
    ...
    this.table
        .getSelectionModel()
        .addListSelectionListener(event -> updateButtons(ScrambleQueueFrame.this.table));

    // up button
    this.buttonUp.addActionListener(
        event -> {
    ...
    // down button
    this.buttonDown.addActionListener(
        event -> {
          JTable table1 = ScrambleQueueFrame.this.table;
          int[] selectedRows = table1.getSelectedRows();
    ...
    // import from file
    this.buttonImportFromFile.addActionListener(
        event -> {
          JFileChooser fileChooser = new JFileChooser();
          fileChooser.setFileFilter(
              new FileNameExtensionFilter(i18n("scramble_queue.scramble_file_description"), "txt"));
          ...
          try {
            scrambles =
                loadScramblesFromFile(
                    fileChooser.getSelectedFile(), puzzleId + "-IMPORTER", scrambleParser);
          } catch (IOException e) {
    ...
    // export to file
    this.buttonExport.addActionListener(
        event -> {
          JFileChooser fileChooser = new JFileChooser();
          ...
          try {
            saveScramblesToFile(selectedScrambles, fileChooser.getSelectedFile());
          } catch (IOException e) {
            JOptionPane.showMessageDialog(
                ScrambleQueueFrame.this,
                String.format(
    ...
    this.buttonImportFromScrambler.addActionListener(
        event -> {
          Scrambler scrambler =
              (Scrambler) ScrambleQueueFrame.this.comboBoxScrambler.getSelectedItem();

          Scramble[] scrambles =
              new Scramble[(Integer) ScrambleQueueFrame.this.spinnerNumberOfScrambles.getValue()];
  ...
  private void updateButtons(JTable table) {
    ...
    this.buttonDown.setEnabled(
        selectedRows.length > 0 && selectedRows[selectedRows.length - 1] != nRows - 1);
  ...
  private Scramble[] loadScramblesFromFile(
      File file, String scramblerId, ScrambleParser scrambleParser) throws IOException {
