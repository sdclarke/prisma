...
public class SolutionEditingDialog extends JDialog {
  ...
  public SolutionEditingDialog(
    ...
    this.textFieldTime.setText(
        SolutionUtils.formatMinutes(
            solution.getTiming().getElapsedTime(),
            configurationManager.getConfiguration("TIMER-PRECISION"),
            false));
    ...
    this.buttonOk.addActionListener(
        event -> {
          ...
          listener.solutionEdited(
              solution.setTiming(timing).setPenalty(penalty).setComment(comment));
  ...
  private void createComponents() {
    setLayout(
        new MigLayout("fill, wrap", "[pref!][fill]", "[pref!]8[pref!]8[pref!]8[pref!]16[bottom]"));
    ...
    // labelComment
    add(new JLabel(i18n("solution_editing.comment")));
