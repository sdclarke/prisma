...
public class SessionSummaryFrame extends JFrame {
  ...
  public SessionSummaryFrame(
    ...
    timerManager.addListener(
        new TimerManager.Listener() {
          ...
          public void precisionChanged(String timerPrecisionId) {
            updateSummary(
                categoryManager.getCurrentCategory(),
                SessionSummaryFrame.this.sessionManager.getSolutions());
          }
        });
    ...
    // copy to clipboard
    this.buttonCopyToClipboard.addActionListener(
        event -> {
  ...
  private void updateSummary(Category currentCategory, Solution[] solutions) {
    ...
    String[] labels = {
      ...
      i18n("session_summary.best_average_of_50"),
    };
    ...
    for (int i = 0; i < statistics.length; i++) {
      ...
      if (solutions.length >= windowSize) {
        ...
        for (int j = 0; j < windowSize; j++) {
          times[j] =
              SolutionUtils.realTime(
                  solutions[windowPosition + j],
    ...
    for (int i = 0; i < realTimes.length; i++) {
      sSolutions[i] =
          SolutionUtils.format(
              realTimes[i], this.configurationManager.getConfiguration("TIMER-PRECISION"), false);
