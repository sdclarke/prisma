...
public class HistoryFrame extends JFrame {
  ...
  public HistoryFrame(
    ...
    categoryManager.addListener(
        new CategoryManager.Listener() {
          ...
          public void categoriesUpdated(Category[] categories, Category currentCategory) {
            setTitle(
                String.format(i18n("history.history_category"), currentCategory.getDescription()));
          }
        });
    ...
    this.buttonAddSolutions.addActionListener(
        e -> {
    ...
    // edit button
    this.buttonEdit.addActionListener(
        e -> {
          Solution[] solutions = solutionManager.getSolutions();
    ...
    this.buttonSelectSession.addActionListener(
        e -> {
          ...
          for (int i = 0, j = 0; i < solutions.length && j < sessionSolutions.length; i++) {
            if (solutions[i].getSolutionId().equals(sessionSolutions[j].getSolutionId())) {
              ...
              HistoryFrame.this.table.scrollRectToVisible(
                  HistoryFrame.this.table.getCellRect(i, 0, true));
              j++;
            }
          }
        });
    ...
    // select none button
    this.buttonSelectNone.addActionListener(
        e -> {
          if (HistoryFrame.this.table.getRowCount() > 0) {
            HistoryFrame.this.table.removeRowSelectionInterval(
                0, HistoryFrame.this.table.getRowCount() - 1);
  ...
  private void createComponents() {
    ...
    // labelAverageOf50
    panelStatistics.add(new JLabel(i18n("history.average_of_50")), "skip 4");
    this.labelAverageOf50 = new JLabel(this.nullTime);
    panelStatistics.add(this.labelAverageOf50, "right");

    // labelBestAverageOf50
    panelStatistics.add(new JLabel(i18n("history.best_average_of_50")), "");
  ...
  private void updateStatistics(Solution[] solutions, final int[] selectedRows) {
    ...
    boolean[] clickable = {
      true, true, true, false, false, true, true, false, false, true, true, false, false, true,
      ...
      if (solutions.length >= measures[i].getMinimumWindowSize()) {
        ...
        labels[i].setText(
            SolutionUtils.formatMinutes(
                measures[i].getValue(),
                this.configurationManager.getConfiguration("TIMER-PRECISION"),
                measures[i].getRound()));
      } else {
                ...
                public void mouseClicked(MouseEvent e) {
                  ...
                  for (int i = 0; i < windowSize; i++) {
                    HistoryFrame.this.table.addRowSelectionInterval(
                        selectedRows[windowPosition + i], selectedRows[windowPosition + i]);
                    HistoryFrame.this.table.scrollRectToVisible(
                        HistoryFrame.this.table.getCellRect(
                            selectedRows[windowPosition + i], 0, true));
                  }
                }
              });
        }
  ...
  private void updateTable(Solution[] solutions) {
    ...
    tableModel.addColumn(i18n("history.comment"));
  ...
  class HeaderListener extends MouseAdapter {
    ...
    public void mousePressed(MouseEvent e) {
      ...
      ((CustomTableModel) header.getTable().getModel()).sortByColumn(sortCol, isAscent);
    }
